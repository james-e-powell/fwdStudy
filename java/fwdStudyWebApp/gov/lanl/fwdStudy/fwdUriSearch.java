...
public class fwdUriSearch extends HttpServlet {
  ...
  static {
        try {
            InputStream in = fwdUriSearch.class.getClassLoader().getResourceAsStream(CONFIG_FILENAME);
            config.load(in);
  ...
  public static void resolveIdentifier (Connection con, PrintWriter outPut, String query, String tableName) {
    try {
      ...
      ResultSet rs=null;
      rs = stmt.executeQuery("select distinct(url) from " + tableName + " where url like \'%"+query+"%\' order by url");
      ...
      while (rs.next()) {
          ...
          outPut.println("<tr><td><a href=\"fwdSimSearch?uri=" + thisUrl + "&table=" + tableName + "\">" + thisUrl + "</a></td></tr>");
      }
