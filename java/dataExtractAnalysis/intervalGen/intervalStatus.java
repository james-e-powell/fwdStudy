...
public class intervalStatus {
  ...
  // usage: java -classpath $CLASSPATH:mysql-connector-java-3.1.12-bin.jar intervalStatus fwd_arxiv2 2
    ...
    try { 
        ...
        while (rs.next()) {
          ...
          while (rs2.next()) {
            ...
            try {
              int typeValCounter = typeCount.get(colPrefix + "xx");
              typeValCounter++;
              typeCount.put(colPrefix + "xx", typeValCounter);
            } catch (Exception e) { 
              typeCount.put(colPrefix + "xx", 1);
            }
            rs2Count++;
